...
const SectionDataEditor = ({ 
  ...
  useEffect(() => {
    if (section && isOpen) {
      ...
      const initialData = section.data || getDefaultDataForComponent(section.componentId);
  ...
  const validateField = (fieldPath, value) => {
    ...
    if (fieldPath.includes('url') && value && !/^https?:\/\/.+/.test(value)) {
      fieldErrors[fieldPath] = 'Please enter a valid URL starting with http:// or https://';
    }
  ...
  const renderField = (key, value, path = '', level = 0) => {
                  ...
                  {typeof item === 'object' && item !== null ? (
                    Object.entries(item).map(([itemKey, itemValue]) => (
                      <div key={itemKey}>
                        ...
                        {renderField(itemKey, itemValue, `${fieldPath}.${index}`, level + 1)}
                      </div>
                    ))
                  ) : (
                    <Input
                      value={item}
                      onChange={(e) => handleArrayItemChange(fieldPath, index, 'value', e.target.value)}
    ...
    const renderInput = () => {
      ...
      if (key.toLowerCase().includes('description') || key.toLowerCase().includes('content') || key.toLowerCase().includes('bio')) {
        return (
          <textarea
            value={value}
            onChange={(e) => handleFieldChange(fieldPath, e.target.value)}
    ...
    return (
      <div key={fieldPath} className={`${indentClass} space-y-2`}>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
          {key.replace(/([A-Z])/g, ' $1').trim()}
          {getRequiredFieldsForComponent(section.componentId).includes(fieldPath) && (
  ...
  const getDefaultArrayItem = (arrayKey) => {
    const defaults = {
      ...
      'plans': { name: '', price: '', period: '', description: '', features: [], cta: '', popular: false },
